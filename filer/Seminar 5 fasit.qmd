...
Regresjon <- lm(freight_cost_usd ~ weight_kilograms +  first_line_designation, data = late_shipments  )
```
...
```{r}
ggplot(late_shipments, aes(y=freight_cost_usd, x=weight_kilograms)) +
  geom_point()
...
Reg_lin <- lm(freight_cost_usd ~ weight_kilograms , data = late_shipments)
summary(Reg_lin)
Reg_log <- lm(log(freight_cost_usd) ~ log(weight_kilograms) , data = late_shipments)
summary(Reg_log)
```
